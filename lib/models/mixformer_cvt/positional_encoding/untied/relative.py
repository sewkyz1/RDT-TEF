...
def generate_2d_concatenated_cross_attention_relative_positional_encoding_index_1(z_shape, x_shape, x_shape_1):
    '''
        z_shape: (z_h, z_w)
        x_shape: (x_h, x_w)
    '''
    z_2d_index_h, z_2d_index_w = torch.meshgrid(torch.arange(z_shape[0]), torch.arange(z_shape[1]))
    x_2d_index_h, x_2d_index_w = torch.meshgrid(torch.arange(x_shape[0]), torch.arange(x_shape[1]))
    x_2d_index_h_1, x_2d_index_w_1 = torch.meshgrid(torch.arange(x_shape_1[0]), torch.arange(x_shape_1[1]))

    z_2d_index_h = z_2d_index_h.flatten(0)
    z_2d_index_w = z_2d_index_w.flatten(0)
    x_2d_index_h = x_2d_index_h.flatten(0)
    x_2d_index_w = x_2d_index_w.flatten(0)
    x_2d_index_h_1 = x_2d_index_h_1.flatten(0)
    x_2d_index_w_1 = x_2d_index_w_1.flatten(0)

    concatenated_2d_index_h = torch.cat((z_2d_index_h, x_2d_index_h))
    concatenated_2d_index_w = torch.cat((z_2d_index_w, x_2d_index_w))

    diff_h = x_2d_index_h_1[:, None] - concatenated_2d_index_h[None, :]
    diff_w = x_2d_index_w_1[:, None] - concatenated_2d_index_w[None, :]

    z_len = z_shape[0] * z_shape[1]
    x_len = x_shape[0] * x_shape[1]
    x_len_1 = x_shape_1[0] * x_shape_1[1]

    a = torch.empty(z_len + x_len, dtype=torch.int64)
    a[: z_len] = 0
    a[z_len:] = 1
    c = a[None, :].repeat(x_len_1, 1)

    diff = torch.stack((diff_h, diff_w, c), dim=-1)
    _, indices = torch.unique(diff.view(x_len_1 * (z_len + x_len), 3), return_inverse=True, dim=0)
    return indices.view(x_len_1, (z_len + x_len))

def generate_2d_concatenated_cross_attention_relative_positional_encoding_index_2(z_shape, x_shape, x_shape_1,z_shape_1):
    '''
        z_shape: (z_h, z_w)
        x_shape: (x_h, x_w)
    '''
    ...
    x_2d_index_h, x_2d_index_w = torch.meshgrid(torch.arange(x_shape[0]), torch.arange(x_shape[1]))
    x_2d_index_h_1, x_2d_index_w_1 = torch.meshgrid(torch.arange(x_shape_1[0]), torch.arange(x_shape_1[1]))
    z_2d_index_h_1, z_2d_index_w_1 = torch.meshgrid(torch.arange(z_shape_1[0]), torch.arange(z_shape_1[1]))
    ...
    x_2d_index_h_1 = x_2d_index_h_1.flatten(0)
    x_2d_index_w_1 = x_2d_index_w_1.flatten(0)
    z_2d_index_h_1 = z_2d_index_h_1.flatten(0)
    z_2d_index_w_1 = z_2d_index_w_1.flatten(0)

    concatenated_2d_index_h = torch.cat((z_2d_index_h, x_2d_index_h))
    concatenated_2d_index_w = torch.cat((z_2d_index_w, x_2d_index_w))
    concatenated_2d_index_h_1 = torch.cat((z_2d_index_h_1, x_2d_index_h_1))
    concatenated_2d_index_w_1 = torch.cat((z_2d_index_w_1, x_2d_index_w_1))

    diff_h = concatenated_2d_index_h_1[:, None] - concatenated_2d_index_h[None, :]
    diff_w = concatenated_2d_index_w_1[:, None] - concatenated_2d_index_w[None, :]

    z_len = z_shape[0] * z_shape[1]
    x_len = x_shape[0] * x_shape[1]
    x_len_1 = x_shape_1[0] * x_shape_1[1]
    z_len_1 = z_shape_1[0] * z_shape_1[1]

    a = torch.empty(z_len + x_len, dtype=torch.int64)
    a[: z_len] = 0
    a[z_len:] = 1
    c = a[None, :].repeat(x_len_1+z_len_1, 1)

    diff = torch.stack((diff_h, diff_w, c), dim=-1)
    _, indices = torch.unique(diff.view((x_len_1+z_len_1) * (z_len + x_len), 3), return_inverse=True, dim=0)
    return indices.view((x_len_1+z_len_1), (z_len + x_len))
