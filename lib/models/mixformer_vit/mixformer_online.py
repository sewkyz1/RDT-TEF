...
class MixFormerOnlineScore(nn.Module):
    ...
    def forward(self, template, online_template, search, run_score_head=True, gt_bboxes=None):  # xun lian hai mei gai
        # search: (b, c, h, w)
        if template.dim() == 5:
    ...
    def set_online_d(self, template, online_template):  # test only use rgb
        if template.dim() == 5:
            template = template.squeeze(0)
        if online_template.dim() == 5:
            online_template = online_template.squeeze(0)
...
def build_mixformer_vit_online_score(cfg, settings=None, train=True) -> MixFormerOnlineScore:
    ...
    backbone_d = get_mixformer_vit(cfg, train)
    # torch.save(backbone_d.state_dict(), '/media/dell/f62e06ef-019a-4728-bd1f-906bcb35e057/MixFormer-main-kyz/backbone_large_d.pth')
    # print(adada)

    # print('/media/dell/f62e06ef-019a-4728-bd1f-906bcb35e057/MixFormer-main-kyz')

    # backbone_d.load_state_dict(torch.load('/media/dell/f62e06ef-019a-4728-bd1f-906bcb35e057/MixFormer-main-kyz/backbone_large_d_trans.pth', map_location='cpu'), strict=True)
    # print('/media/dell/f62e06ef-019a-4728-bd1f-906bcb35e057/MixFormer-main-kyz/backbone_large_d_trans.pth')
    # print('backbone_large_d load success')
    backbone_d.load_state_dict(torch.load('/media/dell/f62e06ef-019a-4728-bd1f-906bcb35e057/MixFormer-main-kyz/backbone_large_d_trans.pth', map_location='cpu'), strict=True)
    print('/media/dell/f62e06ef-019a-4728-bd1f-906bcb35e057/MixFormer-main-kyz/backbone_large_d_trans.pth')
    print('backbone_large_d load success')
    # print(afdaf)
