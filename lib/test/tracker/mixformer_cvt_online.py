...
class MixFormerOnline(BaseTracker):
    def __init__(self, params, dataset_name):
        ...
        self.output_window = hann2d(torch.tensor([20,20]).long(), centered=True).cuda()
    ...
    def track(self, image_rgb, image_d, info: dict = None):
        ...
        with torch.no_grad():
            if self.online_size==1:  # online_size==1 haven't been revised
                ...
                # out_dict, out_, _ = self.network.forward_test(search_rgb, search_d, run_score_head=True) 
                # out_dict, outputs_coord_new, hot_tl, hot_br, attention_map = self.network.forward_test(search_rgb, search_d, run_score_head=True)

        
        # # add hann windows
        # pred_score_map = out_['score_map']
        # response = self.output_window * pred_score_map
        # pred_boxes_center, best_score_center = self.network.box_center_head.cal_bbox(response, out_['size_map'], out_['offset_map'], return_score=True)
        # max_score_center = best_score_center[0][0].item()
        # pred_boxes_center = pred_boxes_center.view(-1, 4)
        # pred_box_center = (pred_boxes_center.mean(dim=0) * self.params.search_size / resize_factor).tolist()  # (cx, cy, w, h) [0,1]
        # self.state_center = clip_box(self.map_box_back(pred_box_center, resize_factor), H, W, margin=10)
        ...
        #             "target_score": pred_score}
            ...
            if self.params.AR:
                ...
                #         "target_score": pred_score}  # , draw, attention_map
                return {"target_bbox": self.state,
                        "target_score": pred_score}, t2-t1  # , draw, attention_map
