...
class MixFormerOnline(BaseTracker):
    ...
    def initialize(self, image_rgb, image_d, info: dict):
        ...
        # self.online_image = image_rgb  # ????????????????????????????????????  # zhu shi kan shi fou you bug  # zhe ge bian liang mei yong shang
    ...
    def track(self, image_rgb, image_d, info: dict = None):
        ...
        if self.frame_id % self.update_interval == 0:
            ...
            self.online_max_template_rgb = self.template_rgb  # chong zhi zui gao fen template wei dang qian template
