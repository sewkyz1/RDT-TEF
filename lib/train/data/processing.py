...
class MixformerProcessing(BaseProcessing):
    ...
    def __call__(self, data: TensorDict):
        ...
        if self.transform['joint'] is not None:
            ...
            # data['search_images'], data['search_anno'], data['search_masks'] = self.transform['joint'](
            #     image=data['search_images'], bbox=data['search_anno'], mask=data['search_masks'], new_roll=False)
            data['template_images_rgb'], data['template_anno'], data['template_masks'] = self.transform['joint'](
                image=data['template_images_rgb'], bbox=data['template_anno'], mask=data['template_masks'])
            data['search_images_rgb'], data['search_anno'], data['search_masks'] = self.transform['joint'](
                image=data['search_images_rgb'], bbox=data['search_anno'], mask=data['search_masks'], new_roll=False)
            data['template_images_depth'], _, _ = self.transform['joint'](
                image=data['template_images_depth'], bbox=data['template_anno'], mask=data['template_masks'])
            data['search_images_depth'], _, _ = self.transform['joint'](
                image=data['search_images_depth'], bbox=data['search_anno'], mask=data['search_masks'], new_roll=False)
        ...
        for s in ['template', 'search']:
            ...
            # Crop image region centered at jittered_anno box and get the attention mask
            crops_rgb, boxes, att_mask, mask_crops = prutils.jittered_center_crop(data[s + '_images_rgb'], jittered_anno,
                                                                              data[s + '_anno'], self.search_area_factor[s],
                                                                              self.output_sz[s], masks=data[s + '_masks'])
            crops_depth, _, _, _ = prutils.jittered_center_crop(data[s + '_images_depth'], jittered_anno,
                                                                              data[s + '_anno'], self.search_area_factor[s],
                                                                              self.output_sz[s], masks=data[s + '_masks'])
            # Apply transforms
            # data[s + '_images'], data[s + '_anno'], data[s + '_att'], data[s + '_masks'] = self.transform[s](
            #     image=crops, bbox=boxes, att=att_mask, mask=mask_crops, joint=False)
            data[s + '_images_rgb'], data[s + '_anno'], data[s + '_att'], data[s + '_masks'] = self.transform[s](
                image=crops_rgb, bbox=boxes, att=att_mask, mask=mask_crops, joint=False)
            data[s + '_images_depth'], _, _, _ = self.transform[s](
                image=crops_depth, bbox=boxes, att=att_mask, mask=mask_crops, joint=False)
