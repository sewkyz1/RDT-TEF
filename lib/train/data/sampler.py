...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
        ...
        # pos_prob=1
        # print("probability of sampling positive class, change this in 'lib/train/data/sampler.py'")
        # print(f"positive_probability={pos_prob}")
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            try:
                ...
                template_frames_rgb, template_frames_depth, template_anno, meta_obj_train = dataset.get_frames(seq_id, template_frame_ids, seq_info_dict)
                search_frames_rgb, search_frames_depth, search_anno, meta_obj_test = dataset.get_frames(seq_id, search_frame_ids, seq_info_dict)

                H, W, _ = template_frames_rgb[0].shape
                ...
                data = TensorDict({'template_images_rgb': template_frames_rgb,
                                   'template_images_depth': template_frames_depth,
                                   'template_anno': template_anno['bbox'],
                                   'template_masks': template_masks,
                                   'search_images_rgb': search_frames_rgb,
                                   'search_images_depth': search_frames_depth,
                                   'search_anno': search_anno['bbox'],
                                   'search_masks': search_masks,
                                   'dataset': dataset.get_name(),
                                   'test_class': meta_obj_test.get('object_class_name')})
    ...
    def getitem_cls(self):
        ...
        valid = False
        label = None
        while not valid:  # if shuju youxiao ze tiaochu xunhuan wancheng yige batch
            # Select a dataset
            dataset = random.choices(self.datasets, self.p_datasets)[0]
                ...
                # negative samples
                    ...
                    if is_video_dataset:
                        ...
                        if search_frame_ids is None:
                            ...
                            search_frames_rgb, search_frames_depth, search_anno, meta_obj_test = self.get_one_search()
                        else:
                    ...
                    H, W, _ = search_frames_rgb[0].shape
                    search_masks = search_anno['mask'] if 'mask' in search_anno else [torch.zeros(
                        (H, W))] * self.num_search_frames
    ...
    def get_one_search(self):
        ...
        return search_frames_rgb, search_frames_depth, search_anno, meta_obj_test
